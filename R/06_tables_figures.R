...
candidate_vars <- c(
  ...
  "share_china", "share_usa", "degree"
)
vars <- intersect(candidate_vars, names(dt1))
...
if ("waiver_any" %in% names(dt1)) {
  ...
  p1 <- ggplot(trend, aes(x = year, y = churn, group = waiver_any,
                            color = factor(waiver_any))) +
    geom_line(size = 1) +
    geom_point() +
    labs(x = NULL, y = "Mean churn (1 - Jaccard)", color = "Exemption granted",
         title = "Average supplier churn over time by exemption status") +
    theme_minimal()

  ggsave(file.path(DIR_OUT_F, "trend_churn_by_waiver.png"), p1, width = 9, height = 5, dpi = 150)
} else {
  # If no waiver data, plot overall trend
  trend_all <- dt1[, .(churn = mean(churn_t_t1, na.rm = TRUE)), by = year]

  p1 <- ggplot(trend_all, aes(x = year, y = churn)) +
    geom_line(size = 1) + geom_point() +
    labs(x = NULL, y = "Mean churn (1 - Jaccard)",
         title = "Average supplier churn over time (all firms)") +
    theme_minimal()

  ggsave(file.path(DIR_OUT_F, "trend_churn_overall.png"), p1, width = 9, height = 5, dpi = 150)
}

# ------------------------------------------------------------
# 3) Network visualization (bipartite firm-supplier for a subset)
# ------------------------------------------------------------
year_pick <- 2019
firms_pick <- sort(unique(dt1$firm_id))[1:min(25, uniqueN(dt1$firm_id))]
...
if (nrow(edges_sub) > 0) {
  ...
  igraph::V(g)$type <- !grepl("^\\d", igraph::V(g)$name)  # adapt detection for gvkeys
...
# ------------------------------------------------------------
# 4) Heatmap of firm-firm overlap weights (W_norm subset)
# ------------------------------------------------------------
firms_in_W <- intersect(firms_pick, rownames(W_norm))
if (length(firms_in_W) >= 5) {
  M <- W_norm[firms_in_W, firms_in_W]
  M_long <- as.data.table(as.table(M))
  M_long <- M_long %>%
    dplyr::rename(firm_i = V1, firm_j = V2, w = N)
  ...
  p_hm <- ggplot(M_long, aes(x = firm_i, y = firm_j, fill = w)) +
    ...
    labs(x = NULL, y = NULL, fill = "Overlap",
         title = "Pre-policy supplier overlap weights (subset)")
